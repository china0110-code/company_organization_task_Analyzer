...
/* ══════════════════════════════════════════════════════════
   PDF エクスポート  —  4象限マップ(p.1) + 結果一覧(p.2〜) を
   1ファイルにまとめて出力する
   ══════════════════════════════════════════════════════════ */
async function exportPDF() {
  ...
  try {
    ...
    drawPageHeader(doc, TITLE, '4象限マップ', stamp, LW, LM);

    // ① canvasの実ピクセル寸法からアスペクト比を計算してPDFに反映
    const canvas   = document.getElementById('qc');
    const imgData  = canvas.toDataURL('image/png', 1.0);
    const pxW      = canvas.width;
    const pxH      = canvas.height;
    const aspect   = pxW / pxH;          // 例: 1.18 なら横長

    const chartTop = 24;
    const maxH     = LH - chartTop - 46; // 凡例・フッター用マージン
    const maxW     = LW - LM * 2 - 60;  // 右の業務リスト用スペース確保

    // アスペクト比を保ったまま収まるサイズを計算
    let chartH, chartW;
    if (maxW / maxH > aspect) {
      // 高さ基準
      chartH = maxH;
    ...
    // 象限凡例バッジ (チャート下)
    const legendY = chartTop + chartH + 5;
    const LEGENDS = [
    ...
    const lw = 40, lh = 7, lgap = 4;
    let lx = chartX;
    LEGENDS.forEach(lg => {
      doc.setFillColor(...lg.bg); doc.setDrawColor(...lg.color);
      doc.setLineWidth(0.5);
      doc.roundedRect(lx, legendY, lw, lh, 2, 2, 'FD');
      doc.setTextColor(...lg.color); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
      doc.text(lg.label, lx + lw / 2, legendY + 4.8, { align: 'center' });
      lx += lw + lgap;
    });

    // ② 業務リスト (チャート右) — 日本語フォント使用
    const listX = chartX + chartW + 8;
    const listW = LW - LM - listX;
    let ly = chartTop + 4;
    setJP(doc, 8, 'bold'); doc.setTextColor(70, 70, 68);
    doc.text('業務一覧', listX, ly); ly += 5;
    doc.setDrawColor(200,198,190); doc.setLineWidth(0.3);
    doc.line(listX, ly, listX + listW, ly); ly += 4;

    eligible.forEach(b => {
      if (ly > LH - LM) return;
      const si = getLv('s', b.sRaw) * getLv('i', b.iRaw);
      const q  = getQ(getLv('p', b.pRaw), si);
      const qc = { qw:[99,153,34], st:[55,138,221], pt:[239,159,39], lp:[136,135,128] }[q];
      doc.setFillColor(...qc);
      doc.circle(listX + 2.5, ly - 1.5, 2, 'F');
      setJP(doc, 7, 'normal'); doc.setTextColor(26,26,24);
      const nm = b.name.length > 14 ? b.name.slice(0,14) + '…' : b.name;
      doc.text(nm, listX + 6.5, ly);
      ly += 5.5;
    });

    drawFooter(doc, LW, LH, LM, stamp, 1);
    ...
    // テーブル列定義
    const COL = [
      { label: '業務名',         w: 30, align: 'left'   },
      { label: '部門',           w: 20, align: 'left'   },
      { label: '適合性',         w: 17, align: 'center' },
      { label: 'P',              w: 8,  align: 'center' },
      { label: 'S',              w: 8,  align: 'center' },
      { label: 'I',              w: 8,  align: 'center' },
      { label: '分類',           w: 24, align: 'center' },
      { label: 'AI活用の可能性', w: contentW - 30-20-17-8-8-8-24, align: 'left' },
    ];
    ...
    const drawTableHeader = (doc, y) => {
      ...
      doc.rect(PM, y, contentW, 7, 'F');
      doc.setDrawColor(200,198,190); doc.setLineWidth(0.3);
      doc.rect(PM, y, contentW, 7, 'S');
      setJP(doc, 7, 'bold'); doc.setTextColor(95,94,90);
      let cx = PM;
      COL.forEach(c => {
    ...
    eligible.forEach((b, ri) => {
      ...
      setJP(doc, 7, 'normal');
      const hintLines = doc.splitTextToSize(hint, COL[7].w - 3);
      const rowH = Math.max(8, hintLines.length * 3.8 + 4);
      ...
      if (y + rowH > PH - 16) {
        ...
        y = drawTableHeader(doc, 26);
      }

      // 行背景
      doc.setFillColor(ri % 2 === 0 ? 255 : 249, ri % 2 === 0 ? 255 : 248, ri % 2 === 0 ? 255 : 244);
      doc.rect(PM, y, contentW, rowH, 'F');
      doc.setDrawColor(220,218,210); doc.setLineWidth(0.25);
      doc.line(PM, y + rowH, PM + contentW, y + rowH);

      let cx = PM;
      const midY = y + rowH / 2;

      // 業務名
      setJP(doc, 7.5, 'bold'); doc.setTextColor(26,26,24);
      const nameLines = doc.splitTextToSize(b.name, COL[0].w - 2);
      doc.text(nameLines, cx + 1.5, midY - (nameLines.length - 1) * 1.9);
      cx += COL[0].w;

      // 部門
      setJP(doc, 7, 'normal'); doc.setTextColor(90,89,85);
      const dept = (b.dept || '—').length > 9 ? (b.dept||'').slice(0,9)+'…' : (b.dept||'—');
      doc.text(dept, cx + 1.5, midY);
      cx += COL[1].w;

      // 適合性バッジ
      const bh = 5, bw = COL[2].w - 4;
      doc.setFillColor(...fc.bg); doc.setDrawColor(...fc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx + 2, midY - bh/2, bw, bh, 1.5, 1.5, 'FD');
      doc.setTextColor(...fc.text); setJP(doc, 6.5, 'bold');
      doc.text(fc.label, cx + COL[2].w/2, midY + 0.8, { align:'center' });
      cx += COL[2].w;

      // P / S / I
      doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.setTextColor(26,26,24);
      ['p','s','i'].forEach((k,ki) => {
        doc.text('Lv.'+getLv(k, b[k+'Raw']), cx + COL[3+ki].w/2, midY, { align:'center' });
        cx += COL[3+ki].w;
      });
      ...
      doc.roundedRect(cx + 2, midY - bh/2, qbw, bh, 1.5, 1.5, 'FD');
      doc.setTextColor(...qc.text); doc.setFont('helvetica','bold'); doc.setFontSize(6.5);
      doc.text(qc.label, cx + COL[6].w/2, midY + 0.8, { align:'center' });
      cx += COL[6].w;

      // AI活用コメント
      doc.setDrawColor(...qc.text); doc.setLineWidth(0.8);
      doc.line(cx + 1.2, y + 2, cx + 1.2, y + rowH - 2);
      doc.setLineWidth(0.25);
      setJP(doc, 6.8, 'normal'); doc.setTextColor(90,89,85);
      doc.text(hintLines, cx + 3.5, y + 5);
...
function embedJPFont(doc) {
  if (typeof IPAG_B64 === 'undefined') return;  // フォントJSが未ロードなら何もしない
  doc.addFileToVFS('ipag.ttf', IPAG_B64);
  doc.addFont('ipag.ttf', 'IPAGothic', 'normal');
  doc.addFont('ipag.ttf', 'IPAGothic', 'bold');  // boldも同じTTFで対応
}

/** 日本語テキスト描画用: IPAGothic → fallback helvetica */
function setJP(doc, size, style) {
  if (typeof IPAG_B64 !== 'undefined') {
    doc.setFont('IPAGothic', style === 'bold' ? 'bold' : 'normal');
  } else {
