...
async function exportPDF() {
  ...
  try {
    ...
    const chartTop = 24;
    const chartH   = LH - chartTop - 44;
    const chartW   = chartH;
    const chartX   = LM + 2;
    doc.addImage(imgData, 'PNG', chartX, chartTop, chartW, chartH);

    // 象限凡例バッジ (チャート下)
    const legendY = chartTop + chartH + 6;
    const LEGENDS = [
    ...
    LEGENDS.forEach(lg => {
      ...
      doc.text(lg.label, lx + lw / 2, legendY + 5.2, { align: 'center' });
      lx += lw + lgap;
    });
    ...
    let ly = chartTop + 4;
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(70, 70, 68);
    ...
    eligible.forEach(b => {
      ...
      const nm = b.name.length > 16 ? b.name.slice(0,16) + '…' : b.name;
    ...
    const drawTableHeader = (doc, y) => {
      ...
      doc.setFont('helvetica','bold'); doc.setFontSize(7); doc.setTextColor(95,94,90);
      let cx = PM;
      COL.forEach(c => {
    ...
    eligible.forEach((b, ri) => {
      ...
      // 業務名
      doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(26,26,24);
      const nameLines = doc.splitTextToSize(b.name, COL[0].w - 2);
      doc.text(nameLines, cx + 1.5, midY - (nameLines.length - 1) * 1.9);
      cx += COL[0].w;

      // 部門
      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(90,89,85);
      ...
      doc.setTextColor(...fc.text); doc.setFont('helvetica','bold'); doc.setFontSize(6.5);
      ...
      doc.setFont('helvetica','normal'); doc.setFontSize(6.8); doc.setTextColor(90,89,85);
...
function drawFooter(doc, PW, PH, MARGIN, stamp, pageNum) {
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.setTextColor(140,138,130);
