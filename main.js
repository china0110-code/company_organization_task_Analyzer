...
async function exportPDF() {
  ...
  try {
    ...
    const chartTop = 22;
    const rightColW = 88; // 右列（象限説明）幅
    const maxW = CW - rightColW - 8;
    const maxH = PH - chartTop - 28; // 凡例・フッター余白
    ...
    // 凡例バッジ（チャート下）
    const legendY = chartTop + chartH + 4;
    const LEGENDS = [
    ...
    // ② 右列: 象限説明カード
    const Q_DEFS = [
      { key:'qw', label:'⚡ Quick Win（左上）',    color:[59,109,17],  bg:[234,243,222], border:[99,153,34],
        desc:'標準ツールで即導入し成功体験を積み上げる。\n議事録要約・翻訳・メール下書きなど\nAIが即時に価値を発揮できる業務。' },
      { key:'st', label:'🎯 Strategic（右上）',     color:[24,95,165],  bg:[230,241,251], border:[55,138,221],
        desc:'予算・リソースを集中投下し独自の\n知能資産を築く。技術承継・特許解析・\n高度な見積自動化など複雑な業務。' },
      { key:'pt', label:'🌱 Potential（左下）',     color:[133,79,11],  bg:[250,238,218], border:[239,159,39],
        desc:'将来の種。技術進化やデータ蓄積を\n待ってから判断する。高度な需要予測・\n未整備データの分析など。' },
      { key:'lp', label:'⛔ Low Priority（右下）',  color:[95,94,90],   bg:[241,239,232], border:[180,178,169],
        desc:'AI化しない領域。無理にAIを当てはめず\n現行通りとする。対面接遇・身体的業務・\n低頻度タスクなど。' },
    ];

    const rcX  = PM + chartW + 10;
    const rcW  = PW - PM - rcX;
    let   ry   = chartTop;
    const cardH = (PH - chartTop - PM - 6) / 4 - 2;
    ...
    Q_DEFS.forEach(qd => {
      ...
      // 説明文
      setJP(doc, 6.5, 'normal'); doc.setTextColor(60,60,58);
      const descLines = doc.splitTextToSize(qd.desc, rcW - 6);
      doc.text(descLines, rcX + 5, ry + 10.5);
    ...
    const D_COL = [
      { label: '業務名',      w: 38, align: 'left' },
      { label: '部門',        w: 24, align: 'left' },
      { label: '適合性',      w: 18, align: 'center' },
      { label: 'P', w: 8, align: 'center' },
      { label: 'S', w: 8, align: 'center' },
      { label: 'I', w: 8, align: 'center' },
      { label: '分類',        w: 26, align: 'center' },
      { label: 'Q1 作業種別',       w: 40, align: 'left' },
      { label: '主な作業内容', w: CW - 38-24-18-8-8-8-26-40, align: 'left' },
    ];
    ...
    let dy = drawTblHdr(D_COL, 22);

    eligible.forEach((b, ri) => {
      const si  = getLv('s', b.sRaw) * getLv('i', b.iRaw);
      const q   = getQ(getLv('p', b.pRaw), si);
      const qc  = Q_COLOR[q];
      const fc  = FIT_COLOR[b.fit];
      const descText = b.desc || '—';
      const q1Text   = (b.q1labels || []).join('、') || '—';

      setJP(doc, 6.5, 'normal');
      const descLines = doc.splitTextToSize(descText, D_COL[8].w - 2);
      const q1Lines   = doc.splitTextToSize(q1Text,   D_COL[7].w - 2);
      const rowH = Math.max(8, Math.max(descLines.length, q1Lines.length) * 3.6 + 4);

      if (dy + rowH > PH - 16) {
        drawFooter(doc, PW, PH, PM, stamp, pageNum++);
        doc.addPage([PW, PH], 'landscape');
        drawPageHeader(doc, TITLE, '業務詳細一覧（続き）', stamp, PW, PM);
        dy = drawTblHdr(D_COL, 22);
      }

      doc.setFillColor(ri % 2 === 0 ? 255 : 249, ri % 2 === 0 ? 255 : 248, ri % 2 === 0 ? 255 : 244);
      doc.rect(PM, dy, CW, rowH, 'F');
      doc.setDrawColor(220,218,210); doc.setLineWidth(0.25);
      doc.line(PM, dy + rowH, PM + CW, dy + rowH);

      let cx = PM;
      const midY = dy + rowH / 2;

      // 業務名
      setJP(doc, 7, 'bold'); doc.setTextColor(26,26,24);
      const nameLines = doc.splitTextToSize(b.name, D_COL[0].w - 2);
      doc.text(nameLines, cx + 1.5, midY - (nameLines.length - 1) * 1.8);
      cx += D_COL[0].w;

      // 部門
      setJP(doc, 6.5, 'normal'); doc.setTextColor(90,89,85);
      doc.text((b.dept||'—').slice(0,10), cx + 1.5, midY);
      cx += D_COL[1].w;

      // 適合性バッジ
      const bh = 5;
      doc.setFillColor(...fc.bg); doc.setDrawColor(...fc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, midY-bh/2, D_COL[2].w-4, bh, 1.5, 1.5, 'FD');
      setJP(doc, 6.2, 'bold'); doc.setTextColor(...fc.text);
      doc.text(fc.label, cx + D_COL[2].w/2, midY + 0.7, { align:'center' });
      cx += D_COL[2].w;

      // P/S/I
      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(26,26,24);
      ['p','s','i'].forEach((k,ki) => {
        doc.text('Lv.'+getLv(k, b[k+'Raw']), cx + D_COL[3+ki].w/2, midY, { align:'center' });
        cx += D_COL[3+ki].w;
      });

      // 分類バッジ
      doc.setFillColor(...qc.bg); doc.setDrawColor(...qc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, midY-bh/2, D_COL[6].w-4, bh, 1.5, 1.5, 'FD');
      doc.setFont('helvetica','bold'); doc.setFontSize(6.5); doc.setTextColor(...qc.text);
      doc.text(qc.label, cx + D_COL[6].w/2, midY + 0.7, { align:'center' });
      cx += D_COL[6].w;

      // Q1 作業種別
      setJP(doc, 6.5, 'normal'); doc.setTextColor(60,59,55);
      doc.text(q1Lines, cx + 1.5, dy + 5);
      cx += D_COL[7].w;

      // 主な作業内容
      doc.setDrawColor(...qc.text); doc.setLineWidth(0.7);
      doc.line(cx + 1, dy + 2, cx + 1, dy + rowH - 2);
      doc.setLineWidth(0.25);
      setJP(doc, 6.5, 'normal'); doc.setTextColor(70,69,65);
      doc.text(descLines, cx + 3, dy + 5);
    ...
    const R_COL = [
      { label: '業務名',         w: 36, align: 'left'   },
      { label: '部門',           w: 22, align: 'left'   },
      { label: '適合性',         w: 17, align: 'center' },
      { label: 'P',              w: 9,  align: 'center' },
      { label: 'S',              w: 9,  align: 'center' },
      { label: 'I',              w: 9,  align: 'center' },
      { label: '分類',           w: 28, align: 'center' },
      { label: 'AI活用の可能性', w: CW - 36-22-17-9-9-9-28, align: 'left' },
    ];
    ...
    eligible.forEach((b, ri) => {
      ...
      setJP(doc, 6.8, 'normal');
      const hintLines = doc.splitTextToSize(hint, R_COL[7].w - 3);
      const rowH = Math.max(8, hintLines.length * 3.6 + 4);
      ...
      doc.setFillColor(ri % 2 === 0 ? 255 : 249, ri % 2 === 0 ? 255 : 248, ri % 2 === 0 ? 255 : 244);
      doc.rect(PM, ry2, CW, rowH, 'F');
      doc.setDrawColor(220,218,210); doc.setLineWidth(0.25);
      doc.line(PM, ry2 + rowH, PM + CW, ry2 + rowH);

      let cx = PM;
      const midY = ry2 + rowH / 2;

      setJP(doc, 7, 'bold'); doc.setTextColor(26,26,24);
      const nameLines = doc.splitTextToSize(b.name, R_COL[0].w - 2);
      doc.text(nameLines, cx + 1.5, midY - (nameLines.length-1)*1.8); cx += R_COL[0].w;

      setJP(doc, 6.5, 'normal'); doc.setTextColor(90,89,85);
      doc.text((b.dept||'—').slice(0,10), cx + 1.5, midY); cx += R_COL[1].w;

      const bh = 5;
      doc.setFillColor(...fc.bg); doc.setDrawColor(...fc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, midY-bh/2, R_COL[2].w-4, bh, 1.5, 1.5, 'FD');
      setJP(doc, 6.2, 'bold'); doc.setTextColor(...fc.text);
      doc.text(fc.label, cx + R_COL[2].w/2, midY+0.7, { align:'center' }); cx += R_COL[2].w;

      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(26,26,24);
      ['p','s','i'].forEach((k,ki) => {
        doc.text('Lv.'+getLv(k,b[k+'Raw']), cx+R_COL[3+ki].w/2, midY, { align:'center' });
        cx += R_COL[3+ki].w;
      });

      doc.setFillColor(...qc.bg); doc.setDrawColor(...qc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, midY-bh/2, R_COL[6].w-4, bh, 1.5, 1.5, 'FD');
      doc.setFont('helvetica','bold'); doc.setFontSize(6.5); doc.setTextColor(...qc.text);
      doc.text(qc.label, cx+R_COL[6].w/2, midY+0.7, { align:'center' }); cx += R_COL[6].w;

      doc.setDrawColor(...qc.text); doc.setLineWidth(0.7);
      doc.line(cx+1.2, ry2+2, cx+1.2, ry2+rowH-2); doc.setLineWidth(0.25);
      setJP(doc, 6.5, 'normal'); doc.setTextColor(90,89,85);
      doc.text(hintLines, cx+3.5, ry2+5);
...
function drawFooter(doc, PW, PH, MARGIN, stamp, pageNum) {
  ...
  doc.text(`${pageNum} ページ  |  出力日: ${stamp}`, PW - MARGIN, PH - 7, { align:'right' });
}
