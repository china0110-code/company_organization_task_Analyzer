...
async function exportPDF(type) {
  ...
  if (type === 'chart') {
    await exportChartPDF(jsPDF, title, stamp);
  } else {
    await exportTablePDF(jsPDF, title, stamp);
  }
...
/* ---- チャートPDF ---- */
async function exportChartPDF(jsPDF, title, stamp) {
  ...
  doc.setFontSize(9);
  doc.text(stamp, PW - MARGIN, 12, { align: 'right' });

  // ── チャート画像 ──
  const chartTop  = 22;
  const chartH    = PH - chartTop - 42; // 下に凡例+マージン分を確保
  const chartW    = chartH;             // 正方形
  ...
  const lw = 44, lh = 8, gap = 6;
  const totalW = legends.length * lw + (legends.length - 1) * gap;
  let lx = (PW - totalW) / 2;
  legends.forEach(lg => {
    doc.setFillColor(...lg.bg);
    doc.setDrawColor(...lg.color);
  ...
  if (eligible.length > 0) {
    ...
    const listW = PW - MARGIN - listX;
    let ly = chartTop + 2;
    doc.setFontSize(7.5);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(90, 90, 88);
    doc.text('業務一覧', listX, ly); ly += 6;
    doc.setLineWidth(0.3);
    doc.setDrawColor(200, 198, 190);
    doc.line(listX, ly - 1, listX + listW, ly - 1); ly += 2;
    ...
    eligible.forEach(b => {
      ...
      // ドット
      doc.setFillColor(...qc);
      doc.circle(listX + 2.5, ly - 1.2, 2.0, 'F');
      // 業務名 (折り返し考慮)
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(26, 26, 24);
      const label = b.name.length > 14 ? b.name.slice(0, 14) + '…' : b.name;
      doc.text(label, listX + 7, ly);
      ly += 5.5;
      if (ly > PH - MARGIN) return; // はみ出し防止
    });
  }

  // ── フッター ──
  drawFooter(doc, PW, PH, MARGIN, stamp);

  doc.save('AI_ユースケース_4象限マップ.pdf');
}

/* ---- 結果一覧PDF ---- */
async function exportTablePDF(jsPDF, title, stamp) {
  const eligible = bizs.filter(b => b.fit !== 'low');
  ...
  doc.setFontSize(10);
  doc.setTextColor(26, 26, 24);
  doc.text('AI活用業務 優先順位結果一覧', MARGIN, y); y += 8;

  // ── テーブル定義 ──
  const COL = [
    { label: '業務名',        w: 32, align: 'left' },
    ...
    { label: 'AI活用の可能性', w: contentW - 32-22-18-9-9-9-24, align: 'left' },
  ];

  // ── テーブルヘッダー ──
  const ROW_H = 7, TH_H = 7;
  doc.setFillColor(241, 239, 232);
  doc.rect(MARGIN, y, contentW, TH_H, 'F');
  doc.setDrawColor(200, 198, 190);
  doc.setLineWidth(0.3);
  doc.rect(MARGIN, y, contentW, TH_H, 'S');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7.5);
  ...
  eligible.forEach((b, ri) => {
    ...
    // ヒントの折り返し
    doc.setFontSize(7);
    const hintLines = doc.splitTextToSize(hint, COL[7].w - 2);
    const rowH      = Math.max(ROW_H, hintLines.length * 4 + 4);
    ...
    // 行背景
    doc.setFillColor(ri % 2 === 0 ? 255 : 250, ri % 2 === 0 ? 255 : 249, ri % 2 === 0 ? 255 : 245);
    ...
    doc.text(nameLines, cx + 1.5, midY - (nameLines.length - 1) * 2); cx += COL[0].w;

    // 部門
    const deptTxt = b.dept || '—';
    doc.text(deptTxt.length > 8 ? deptTxt.slice(0,8)+'…' : deptTxt, cx + 1.5, midY); cx += COL[1].w;

    // 適合性バッジ
    const bw = COL[2].w - 4, bh = 5.5;
    const bx = cx + 2, by = midY - bh / 2;
    doc.setFillColor(...fc.bg);
    doc.setDrawColor(...fc.text);
    doc.setLineWidth(0.3);
    doc.roundedRect(bx, by, bw, bh, 1.5, 1.5, 'FD');
    doc.setTextColor(...fc.text);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    ...
    // 分類バッジ
    const qbw = COL[6].w - 4;
    const qbx = cx + 2, qby = midY - bh / 2;
    doc.setFillColor(...qc.bg);
    doc.setDrawColor(...qc.text);
    doc.setLineWidth(0.3);
    doc.roundedRect(qbx, qby, qbw, bh, 1.5, 1.5, 'FD');
    doc.setTextColor(...qc.text);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    ...
    doc.line(cx + 1, y + 2, cx + 1, y + rowH - 2);
    doc.setLineWidth(0.25);
    doc.text(hintLines, cx + 3, y + 5);
  ...
  doc.save('AI_ユースケース_結果一覧.pdf');
}

/* ── フッター描画ヘルパー ─────────────────────────────── */
function drawFooter(doc, PW, PH, MARGIN, stamp) {
  doc.setDrawColor(200, 198, 190);
  doc.setLineWidth(0.3);
  ...
  doc.text(`出力日: ${stamp}`, PW - MARGIN, PH - 7, { align: 'right' });
}
