...
/* ══════════════════════════════════════════════════════════
   ...
   p.1: 4象限マップ + 象限説明カード
   p.2: 業務詳細一覧（Q1作業種別・主な作業内容）
   p.3〜: 優先順位 結果一覧（AI活用の可能性付き）
   ══════════════════════════════════════════════════════════ */
async function exportPDF() {
  ...
  const CW = PW - PM * 2;
  const LINE_H = 3.8; // 本文1行あたりの高さ(mm)
  ...
  // ── テーブルヘッダー描画ユーティリティ ──
  // ※ cols を引数で受け取り、クロージャ参照バグを回避
  const drawTblHdr = (cols, y) => {
    doc.setFillColor(241,239,232);
    doc.rect(PM, y, CW, 7, 'F');
    doc.setDrawColor(200,198,190); doc.setLineWidth(0.3);
    doc.rect(PM, y, CW, 7, 'S');
    setJP(doc, 6.8, 'bold'); doc.setTextColor(95,94,90);
    let cx = PM;
    cols.forEach(c => {
      const tx = c.align === 'center' ? cx + c.w/2 : cx + 1.5;
      doc.text(c.label, tx, y + 4.8, { align: c.align === 'center' ? 'center' : 'left' });
      cx += c.w;
    });
    return y + 7;
  };
  ...
  try {
    ...
    const chartTop = 22;
    const rcW     = 86;  // 右列幅（象限説明）
    const maxW    = CW - rcW - 10;
    const maxH    = PH - chartTop - 26;
    let chartW, chartH;
    if (maxW / maxH > aspect) { chartH = maxH; chartW = maxH * aspect; }
    else                      { chartW = maxW; chartH = maxW / aspect; }
    doc.addImage(imgData, 'PNG', PM, chartTop, chartW, chartH);

    // 凡例バッジ（チャート下）
    const legendY = chartTop + chartH + 3;
    const LEGENDS = [
      { label:'Quick Win',    color:[99,153,34],   bg:[234,243,222] },
      { label:'Strategic',    color:[55,138,221],  bg:[230,241,251] },
      { label:'Potential',    color:[239,159,39],  bg:[250,238,218] },
      { label:'Low Priority', color:[136,135,128], bg:[241,239,232] },
    ];
    const lw=35, lh=6, lgap=3;
    let lx=PM;
    LEGENDS.forEach(lg => {
      doc.setFillColor(...lg.bg); doc.setDrawColor(...lg.color); doc.setLineWidth(0.4);
      doc.roundedRect(lx, legendY, lw, lh, 1.5, 1.5, 'FD');
      doc.setTextColor(...lg.color); doc.setFont('helvetica','bold'); doc.setFontSize(7.5);
      doc.text(lg.label, lx+lw/2, legendY+4.1, { align:'center' });
      lx += lw+lgap;
    });

    // ① 右列: 象限説明カード（絵文字なし・splitTextToSize で折り返し）
    const Q_DEFS = [
      { key:'qw', label:'Quick Win（左上）',
        color:[59,109,17],  bg:[234,243,222], border:[99,153,34],
        desc:'標準ツールで即導入し成功体験を積み上げる領域。議事録要約・翻訳・メール下書きなど、AIが即時に価値を発揮できる業務。' },
      { key:'st', label:'Strategic（右上）',
        color:[24,95,165],  bg:[230,241,251], border:[55,138,221],
        desc:'予算・リソースを集中投下し独自の知能資産を築く領域。技術承継・特許解析・高度な見積自動化など複雑な業務。' },
      { key:'pt', label:'Potential（左下）',
        color:[133,79,11],  bg:[250,238,218], border:[239,159,39],
        desc:'将来の種となる領域。技術進化やデータ蓄積を待ってから判断する。高度な需要予測・未整備データの分析など。' },
      { key:'lp', label:'Low Priority（右下）',
        color:[95,94,90],   bg:[241,239,232], border:[180,178,169],
        desc:'AI化しない領域。無理にAIを当てはめず現行通りとする。対面接遇・身体的業務・低頻度タスクなど。' },
    ];

    const rcX   = PM + chartW + 10;
    const cardH = (PH - chartTop - PM - 8) / 4 - 2;
    let   ry    = chartTop;
    const textW = rcW - 7; // テキスト折り返し幅

    Q_DEFS.forEach(qd => {
      doc.setFillColor(...qd.bg);
      doc.setDrawColor(...qd.border);
      doc.setLineWidth(0.4);
      doc.roundedRect(rcX, ry, rcW, cardH, 2, 2, 'FD');
      doc.setFillColor(...qd.border);
      doc.roundedRect(rcX, ry, 2.5, cardH, 1, 1, 'F');

      // ① タイトル（絵文字なしで文字化け防止）
      setJP(doc, 7.5, 'bold'); doc.setTextColor(...qd.color);
      doc.text(qd.label, rcX + 5, ry + 5.5);

      // ① 説明文（splitTextToSizeで適切に折り返し）
      setJP(doc, 6.2, 'normal'); doc.setTextColor(50,50,48);
      const dLines = doc.splitTextToSize(qd.desc, textW);
      doc.text(dLines, rcX + 5, ry + 10.5);
    ...
    /* ══════════════════════════════════════════════════════
       p.2: 業務詳細一覧（③重複バグ修正：cols を引数渡し）
       ══════════════════════════════════════════════════════ */
    doc.addPage([PW, PH], 'landscape');
    drawPageHeader(doc, TITLE, '業務詳細一覧', stamp, PW, PM);

    // ② 列幅を合計 CW に収まるよう調整
    const W_NAME=32, W_DEPT=20, W_FIT=16, W_P=8, W_S=8, W_I=8, W_Q=26;
    const W_Q1=44, W_DESC=CW-W_NAME-W_DEPT-W_FIT-W_P-W_S-W_I-W_Q-W_Q1;
    const D_COL = [
      { label:'業務名',      w:W_NAME, align:'left'   },
      { label:'部門',        w:W_DEPT, align:'left'   },
      { label:'適合性',      w:W_FIT,  align:'center' },
      { label:'P',           w:W_P,    align:'center' },
      { label:'S',           w:W_S,    align:'center' },
      { label:'I',           w:W_I,    align:'center' },
      { label:'分類',        w:W_Q,    align:'center' },
      { label:'Q1 作業種別', w:W_Q1,   align:'left'   },
      { label:'主な作業内容',w:W_DESC,  align:'left'   },
    ];

    let dy = drawTblHdr(D_COL, 22); // ③ cols を引数で渡す
    ...
    eligible.forEach((b, ri) => {
      ...
      // ② 折り返し幅を列幅から余白を引いた値に統一
      const q1Lines   = doc.splitTextToSize(q1Text,   W_Q1  - 3);
      const descLines = doc.splitTextToSize(descText,  W_DESC - 4);
      const rowH = Math.max(9, Math.max(q1Lines.length, descLines.length) * LINE_H + 4);

      if (dy + rowH > PH - 16) {
        drawFooter(doc, PW, PH, PM, stamp, pageNum++);
        doc.addPage([PW, PH], 'landscape');
        drawPageHeader(doc, TITLE, '業務詳細一覧（続き）', stamp, PW, PM);
        dy = drawTblHdr(D_COL, 22); // ③ 同じく引数渡し
      }

      drawRowBg(dy, rowH, ri);
      const TOP = dy + 5; // テキスト描画を上端基準に統一
      const MID = dy + rowH / 2;
      let cx = PM;

      setJP(doc, 7, 'bold'); doc.setTextColor(26,26,24);
      const nameL = doc.splitTextToSize(b.name, W_NAME-2);
      doc.text(nameL, cx+1.5, TOP); cx += W_NAME;

      setJP(doc, 6.5, 'normal'); doc.setTextColor(90,89,85);
      doc.text((b.dept||'—').slice(0,9), cx+1.5, MID); cx += W_DEPT;

      const bh=5;
      doc.setFillColor(...fc.bg); doc.setDrawColor(...fc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, MID-bh/2, W_FIT-4, bh, 1.5,1.5,'FD');
      setJP(doc, 6, 'bold'); doc.setTextColor(...fc.text);
      doc.text(fc.label, cx+W_FIT/2, MID+0.7, {align:'center'}); cx+=W_FIT;

      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(26,26,24);
      ['p','s','i'].forEach((k,ki)=>{
        doc.text('Lv.'+getLv(k,b[k+'Raw']), cx+[W_P,W_S,W_I][ki]/2, MID, {align:'center'});
        cx += [W_P,W_S,W_I][ki];
      });

      doc.setFillColor(...qc.bg); doc.setDrawColor(...qc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, MID-bh/2, W_Q-4, bh, 1.5,1.5,'FD');
      doc.setFont('helvetica','bold'); doc.setFontSize(6.5); doc.setTextColor(...qc.text);
      doc.text(qc.label, cx+W_Q/2, MID+0.7, {align:'center'}); cx+=W_Q;

      // Q1 作業種別（上端基準で描画）
      setJP(doc, 6.5, 'normal'); doc.setTextColor(60,59,55);
      doc.text(q1Lines, cx+1.5, TOP); cx+=W_Q1;

      // 主な作業内容（左アクセントライン付き、上端基準）
      doc.setDrawColor(...qc.text); doc.setLineWidth(0.6);
      doc.line(cx+1, dy+2, cx+1, dy+rowH-2); doc.setLineWidth(0.25);
    ...
    /* ══════════════════════════════════════════════════════
       p.3〜: 優先順位 結果一覧（⑤AI活用の可能性 追加）
       ══════════════════════════════════════════════════════ */
    doc.addPage([PW, PH], 'landscape');
    drawPageHeader(doc, TITLE, '優先順位 結果一覧', stamp, PW, PM);

    const W_RNAME=30, W_RDEPT=18, W_RFIT=15, W_RP=8, W_RS=8, W_RI=8, W_RCAT=25;
    const W_RHINT = CW - W_RNAME - W_RDEPT - W_RFIT - W_RP - W_RS - W_RI - W_RCAT;
    const R_COL = [
      { label:'業務名',         w:W_RNAME, align:'left'   },
      { label:'部門',           w:W_RDEPT, align:'left'   },
      { label:'適合性',         w:W_RFIT,  align:'center' },
      { label:'P',              w:W_RP,    align:'center' },
      { label:'S',              w:W_RS,    align:'center' },
      { label:'I',              w:W_RI,    align:'center' },
      { label:'分類',           w:W_RCAT,  align:'center' },
      { label:'AI活用の可能性', w:W_RHINT, align:'left'   },
    ];

    let ry2 = drawTblHdr(R_COL, 22);
    ...
    eligible.forEach((b, ri) => {
      ...
      setJP(doc, 6.5, 'normal');
      // ⑤ AI活用の可能性テキストを確実に折り返し
      const hintLines = doc.splitTextToSize(hint, W_RHINT - 4);
      const rowH = Math.max(9, hintLines.length * LINE_H + 4);

      if (ry2 + rowH > PH - 16) {
        drawFooter(doc, PW, PH, PM, stamp, pageNum++);
        doc.addPage([PW, PH], 'landscape');
        drawPageHeader(doc, TITLE, '優先順位 結果一覧（続き）', stamp, PW, PM);
        ry2 = drawTblHdr(R_COL, 22);
      }

      drawRowBg(ry2, rowH, ri);
      const TOP = ry2 + 5;
      const MID = ry2 + rowH / 2;
      let cx = PM;

      setJP(doc, 7, 'bold'); doc.setTextColor(26,26,24);
      const nameL = doc.splitTextToSize(b.name, W_RNAME-2);
      doc.text(nameL, cx+1.5, TOP); cx+=W_RNAME;

      setJP(doc, 6.5, 'normal'); doc.setTextColor(90,89,85);
      doc.text((b.dept||'—').slice(0,9), cx+1.5, MID); cx+=W_RDEPT;

      const bh=5;
      doc.setFillColor(...fc.bg); doc.setDrawColor(...fc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, MID-bh/2, W_RFIT-4, bh, 1.5,1.5,'FD');
      setJP(doc, 6, 'bold'); doc.setTextColor(...fc.text);
      doc.text(fc.label, cx+W_RFIT/2, MID+0.7, {align:'center'}); cx+=W_RFIT;

      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(26,26,24);
      ['p','s','i'].forEach((k,ki)=>{
        doc.text('Lv.'+getLv(k,b[k+'Raw']), cx+[W_RP,W_RS,W_RI][ki]/2, MID, {align:'center'});
        cx += [W_RP,W_RS,W_RI][ki];
      });

      doc.setFillColor(...qc.bg); doc.setDrawColor(...qc.text); doc.setLineWidth(0.3);
      doc.roundedRect(cx+2, MID-bh/2, W_RCAT-4, bh, 1.5,1.5,'FD');
      doc.setFont('helvetica','bold'); doc.setFontSize(6.5); doc.setTextColor(...qc.text);
      doc.text(qc.label, cx+W_RCAT/2, MID+0.7, {align:'center'}); cx+=W_RCAT;

      // ⑤ AI活用の可能性（左アクセントライン付き、上端基準）
      doc.setDrawColor(...qc.text); doc.setLineWidth(0.6);
      doc.line(cx+1, ry2+2, cx+1, ry2+rowH-2); doc.setLineWidth(0.25);
      setJP(doc, 6.5, 'normal'); doc.setTextColor(70,69,65);
