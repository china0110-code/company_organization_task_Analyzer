...
async function exportPDF() {
  ...
  // ※ doc は try ブロックで生成するため、関数内で参照する際は引数で渡す
  ...
  try {
    ...
    let dy = drawTblHdr(doc, D_COL, 22); // ③ cols を引数で渡す
    ...
    eligible.forEach((b, ri) => {
      ...
      if (dy + rowH > PH - 16) {
        ...
        dy = drawTblHdr(doc, D_COL, 22); // ③ 同じく引数渡し
      }
    ...
    let ry2 = drawTblHdr(doc, R_COL, 22);
    ...
    eligible.forEach((b, ri) => {
      ...
      if (ry2 + rowH > PH - 16) {
        ...
        ry2 = drawTblHdr(doc, R_COL, 22);
      }

      drawRowBg(doc, ry2, rowH, ri);
      const TOP = ry2 + 5;
